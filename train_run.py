...
tf.app.flags.DEFINE_integer('num_epochs', 100, 'The number of training epoch.')
...
def main(_):
    logging_path = FLAGS.dataset_dir + "output/log/train_{}.log".format(time.strftime(time.strftime("%m%d-%H_%M_%S", time.localtime())))
    ...
    label, fea, neig_id, neig_value = model.inputs(FLAGS.dataset_dir + "train.tfrecord")
    ...
    with tf.Session() as sess:
        ...
        if FLAGS.init_iter > 0:
            saver.restore(sess, FLAGS.dataset_dir + 'output/model/model-{}'.format(FLAGS.init_iter))
